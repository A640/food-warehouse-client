<template>
    ...
    <popup :edit="edit" v-slot="scope" ref="popup" >
        ...
        <div class="cell ">
            <p class="section-title">{{ $t("warehouse.warehouse") }}</p>
        </div>

        <edit-warehouse
            :pid="warehouse_id" 
            :validate="validate_trigger_w" 
            @dataUpdate="updateDataWarehouse" 
...
import EditWarehouse from '@/components/Editors/WarehouseEditor.vue'
import EditAddress from '@/components/Editors/AddressEditor.vue'

export default {

    name:'Add-Edit-WarehousePopupContent',
    ...
    methods: {
        ...
        validate(){
            this.w_validated = false;
            this.ad_validated = false;
            this.validate_trigger_w = true;
            this.validate_trigger_ad = true;
        },

        validateP(value){
            this.w_validated = value;
            this.validate_trigger_w = false;
        ...
        validateAD(value){
            ...
            if(value == true && this.w_validated == true){
                this.eSave();
            }
        },
        ...
        eSave(){
            ...
            if(this.edit){
                ...
                console.log(obj);
                this.$store.dispatch('editWarehouse',obj)
                    .then( () => {
                ...
                console.log("save new warehouse attempt");
                console.log(obj);
                this.$store.dispatch('addWarehouse',obj)
                    .then( () => {
    ...
    mounted() {
        if(this.warehouse_id != -1){
            this.$store.dispatch('getWarehouseAddress',this.warehouse_id)
            .then( (address) => {
